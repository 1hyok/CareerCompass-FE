 ...
 *
 * ## 왜 Effect 타입이 없는가 (#244 판정)
 *
 * MVI 가 요구하는 것은 「일회성 효과를 상태 전이에서 분리한다」 까지고, 그것을 `Channel` 로
 * 나르는 것은 구현 선택이다. 이 저장소의 ViewModel 은 처음부터 이벤트를 `Channel` 로 쏘지 않고
 * [UiState] 의 nullable 신호로 흡수해 왔다(`LoginViewState.failure`·`pendingNavigation`). producer(ViewModel)가 consumer(UI)보다
 ...
 *
 * 일회성 신호도 nullable 필드로 여기에 함께 담는다. 별도 스트림을 두지 않는다.
 */
interface UiState
