 ...
 * 소셜 로그인 화면의 상태 배선.
 *
 * 카카오·Google SDK 는 Activity 를 요구하므로 토큰 획득은 화면 쪽 [rememberSocialLoginLauncher] 가 만들고,
 * [LoginViewModel] 에는 「토큰을 가져오는 일」만 넘긴다 — 진행 중인 시도는 컴포지션이 아니라 ViewModel 이 들고
 * 있어야 화면이 재생성돼도 잠금이 풀린다(#147). Activity 가 없는 호스트(프리뷰 등)에서는 SDK 를 부르지 않고
 * 실패로 처리한다.
 *
