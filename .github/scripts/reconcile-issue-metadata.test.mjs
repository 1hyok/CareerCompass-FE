...
test("reads exact issue-form sections with CRLF input", () => {
    const body = formBody("enhancement", "profile").replaceAll("\n", "\r\n");
    assert.equal(readFormSection(body, "작업 유형"), "enhancement — 선택한 작업 유형");
    assert.equal(readFormSection(body, "주 담당 모듈"), "profile — 선택한 주 담당 모듈");
});
...
test("replaces wrong type, area, legacy internal labels and assignees while preserving operational labels", () => {
    const inspection = inspectIssue(issue({
        ...
        assignees: [{ login: "1hyok" }, { login: "outsider" }],
    }));
    assert.equal(inspection.status, "valid");
    assert.equal(inspection.needsUpdate, true);
    assert.deepEqual(inspection.labels, ["P1", "bug", "area:editor"]);
    assert.deepEqual(inspection.assignees, [ASSIGNEE_BY_MODULE.editor]);
});
...
test("corrects metadata and verifies the persisted postcondition", async () => {
    const original = issue({
        body: formBody("documentation", "foryou"),
    ...
    assert.deepEqual(result, {
        ...
        action: "corrected",
        label: "documentation",
        areaLabel: "area:foryou",
        assignee: ASSIGNEE_BY_MODULE.foryou,
    });
    assert.deepEqual(
        fake.currentIssue().labels,
        [{ name: "P0" }, { name: "documentation" }, { name: "area:foryou" }],
    );
    assert.deepEqual(fake.currentIssue().assignees, [{ login: ASSIGNEE_BY_MODULE.foryou }]);
