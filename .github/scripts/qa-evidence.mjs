...
const JSON_PRIVATE_NUMERIC_ID_PATTERN = /(["'](?:(?:careercompass|receiver|content|account|verification|user|record|delivery)[_-]?id|sub|subject)["']\s*:\s*)["']?\d+["']?/gi;
const PRIVATE_NUMERIC_ID_PATTERN = /(\b(?:(?:careercompass|receiver|content|account|verification|user|record|delivery)[_-]?id|sub|subject)\s*[:=#]\s*)\d+\b/gi;
...
const SENSITIVE_KEY_PATTERN = /^(?:account|careercompass_id|authorization|avd|cookie|credential|email|fixture|password|passcode|qa_account|receiverid|secret|serial|server|session|token)$/i;
