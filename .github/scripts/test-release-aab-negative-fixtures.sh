...
fixture_root="$(mktemp -d "${TMPDIR:-/tmp}/careercompass-release-negative.XXXXXX")"
...
printf '\n-careercompass-invalid-keep-rule\n' >> "${rules_path}"
set +e
...
if ! grep -Eiq 'careercompass-invalid-keep-rule|unknown option' <<< "${invalid_rule_output}"; then
