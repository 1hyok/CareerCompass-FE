...
test("every registered module has a ktlintCheck task", async () => {
    ...
    // 그 태스크가 실재하는지는 확인하지 않는다. 모든 모듈이 자기 build.gradle.kts 를
...
test("the convention chain that carries ktlint is discovered transitively", async () => {
    ...
    assert.ok(bearing.has("careercompass.android.lint"), "직접 적용 시드를 못 찾았다");
    assert.ok(bearing.has("careercompass.android.library"), "1단계 전이를 못 따라갔다");
    assert.ok(bearing.has("careercompass.android.library.compose"), "2단계 전이를 못 따라갔다");
    assert.ok(bearing.has("careercompass.jvm.library"), "JVM 쪽 시드를 못 찾았다");
    assert.ok(bearing.has("careercompass.jvm.domain"), "JVM 전이를 못 따라갔다");
    // ktlint 와 무관한 컨벤션까지 물들면 판정이 무의미해진다.
    assert.ok(!bearing.has("careercompass.kover"), "무관한 컨벤션이 섞였다");
});
