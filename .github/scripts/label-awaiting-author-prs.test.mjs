...
const REPO = "Team-CareerCompass/CareerCompass-FE";
...
test("입장 게이트 조회는 GraphQL search 에서 같은 작성자의 열린 PR 만 페이지 처리한다", async () => {
    ...
    const api = async (apiPath, options) => {
        ...
        assert.equal(
            ...
            "repo:Team-CareerCompass/CareerCompass-FE is:pr is:open author:AuThOr",
        );
...
test("draft·봇·fork 는 판정 대상이 아니다", () => {
    ...
    const fork = pullRequest({
        headRepository: { nameWithOwner: "someone/CareerCompass-FE" },
        reviews: { nodes: [changesRequested()] },
    });
