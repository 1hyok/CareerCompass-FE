...
test("실제 형태의 16개 결과에서 세 실패의 메시지와 테스트 소스 위치를 요약한다", () => {
    ...
    assert.match(markdown, /EditorFlowAndroidTest#scheduledSave_failureThenRetry_keepsExactPayload/);
    assert.match(markdown, /EditorFlowAndroidTest\.kt:97/);
...
test("CLI는 summary와 annotation을 먼저 남기고 XML testcase 실패를 nonzero로 만든다", async () => {
    ...
    try {
        ...
        assert.equal(execution.status, 1);
        assert.match(execution.stdout, /::error .*EditorFlowAndroidTest/);
