...
test("JVM and Android test plugins are excluded from Android lint task selection", () => {
    ...
    assert.equal(isAndroidModuleBuild('plugins { id("careercompass.jvm.library") }'), false);
    assert.equal(isAndroidModuleBuild('plugins { id("careercompass.jvm.domain") }'), false);
    ...
    assert.equal(isAndroidModuleBuild('plugins { id("careercompass.android.library") }'), true);
...
test("konsist guard changes run the guard they touch", () => {
    ...
    const impact = resolvePrImpact(
        ["konsist/src/test/kotlin/com/careercompass/konsist/LayerDependencyKonsistTest.kt"],
        modules,
        dependencies,
    );
