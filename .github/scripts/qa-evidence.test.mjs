...
function rawFixture() {
    return {
        ...
        app: {
            ...
            note: "기기 내부 /data/user/0/com.cambridge.careercompass_fe/files/session.json 확인",
        },
        device: {
...
test("sanitizes account, endpoint, device, session, and screenshot identifiers", () => {
    ...
    assert.doesNotMatch(record.scenario, /careercompassId=39/);
...
test("validates the committed public QA evidence corpus", () => {
    ...
    assert.equal(manifest.initial_migration_count, 0);
