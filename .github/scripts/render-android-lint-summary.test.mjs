...
test("derives the Gradle module path from the report location", () => {
    ...
    assert.equal(
        moduleOf("/workspace", "/workspace/feature/careercompass/presentation/build/reports/lint-results-debug.xml"),
...
test("collects one report per module across core and feature trees", async () => {
    ...
    try {
        ...
        await writeReport(root, path.join("feature", "careercompass", "presentation"), REPORT);
        ...
        assert.deepEqual(
            ...
            [":core:common", ":feature:careercompass:presentation"],
        );
    } finally {
