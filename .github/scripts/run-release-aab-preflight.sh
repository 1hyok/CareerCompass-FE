...
: "${CAREERCOMPASS_CI_RELEASE_KEYSTORE:?CAREERCOMPASS_CI_RELEASE_KEYSTORE is required}"
: "${CAREERCOMPASS_CI_RELEASE_STORE_PASSWORD_FILE:?CAREERCOMPASS_CI_RELEASE_STORE_PASSWORD_FILE is required}"
: "${CAREERCOMPASS_CI_RELEASE_KEY_PASSWORD_FILE:?CAREERCOMPASS_CI_RELEASE_KEY_PASSWORD_FILE is required}"
: "${CAREERCOMPASS_CI_RELEASE_KEY_ALIAS:?CAREERCOMPASS_CI_RELEASE_KEY_ALIAS is required}"

[[ "${CAREERCOMPASS_CI_CONFIG_MODE:-}" == "stub" ]] || {
    echo "Release preflight requires the #842 secretless CI fixture." >&2
    exit 1
}
[[ "${CAREERCOMPASS_CI_RELEASE_SIGNING_MODE:-}" == "ephemeral" ]] || {
...
for required_file in \
    "${BUNDLETOOL_JAR}" \
    "${CAREERCOMPASS_CI_RELEASE_KEYSTORE}" \
    "${CAREERCOMPASS_CI_RELEASE_STORE_PASSWORD_FILE}" \
    "${CAREERCOMPASS_CI_RELEASE_KEY_PASSWORD_FILE}"; do
    [[ -s "${required_file}" ]] || {
...
private_dir="$(mktemp -d "${TMPDIR:-/tmp}/careercompass-release-preflight.XXXXXX")"
...
java -jar "${BUNDLETOOL_JAR}" build-apks \
    ...
    --mode=universal \
    --overwrite \
    --ks="${CAREERCOMPASS_CI_RELEASE_KEYSTORE}" \
    --ks-pass="file:${CAREERCOMPASS_CI_RELEASE_STORE_PASSWORD_FILE}" \
    --ks-key-alias="${CAREERCOMPASS_CI_RELEASE_KEY_ALIAS}" \
    --key-pass="file:${CAREERCOMPASS_CI_RELEASE_KEY_PASSWORD_FILE}"
