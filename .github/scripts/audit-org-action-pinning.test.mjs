...
test("summarizes per repository and reports incompatibility when any floating reference remains", () => {
    const entries = [
        { repository: "Team-CareerCompass/CareerCompass-FE", ...classifyActionReference(PINNED) },
        {
            repository: "Team-CareerCompass/CareerCompass-FE",
            ...classifyActionReference("./.github/actions/setup-ci-config"),
        },
        {
            repository: "CareerCompass/CareerCompass-BE",
    ...
    assert.deepEqual(
        ...
        [
            ["CareerCompass/CareerCompass-BE", 0, 1],
            ["Team-CareerCompass/CareerCompass-FE", 1, 0],
        ],
    );
...
test("requires an organization and accepts the optional output and api overrides", () => {
    assert.deepEqual(parseArguments(["--org", "CareerCompass"]), {
        org: "CareerCompass",
        output: null,
    ...
    assert.deepEqual(
        ...
        { org: "CareerCompass", output: "/tmp/audit.json", apiUrl: "https://example.test" },
    );
