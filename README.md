...
| [`:app`](app) · [`:baselineprofile`](baselineprofile) | 시작 목적지 ViewModel, 온보딩·피드 그래프와 하단 탭을 잇는 앱 셸, 공고 상세 딥링크(`careercompass://postings/{id}`, 인증 뒤 적용), Kakao SDK 초기화, Crashlytics `ErrorReporter`, 계측 테스트의 Hilt fake 주입(`AppNavigationAndroidTest`·API 경계·접근성 smoke), Baseline/Startup Profile | 다른 담당 모듈 탭·알림의 실제 진입점, 로그아웃 진입(마이 탭) |
