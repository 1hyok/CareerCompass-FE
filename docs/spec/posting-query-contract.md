...
원문을 빼는 이유는 두 가지다. 첫째, 화면이 약속한 범위가 제목과 키워드까지다(`feed_search_placeholder`). 본문은 어디에서도 약속한 적이 없다. 둘째, 키워드는 상세 화면이 칩으로 그려 주는 짧은 목록이라(`PostingKeywordsCard`) 왜 걸렸는지 사용자가 확인할 자리가 있지만, 원문은 수집한 본문 전체라 「지원」 같은 흔한 말 하나로 거의 모든 공고가 걸려 결과가 무너진다. 목록 카드가 공고에서 그리는 글은 제목뿐이므로(유형·게시판 배지와 마감일·수집일·적합도는 값이지 본문이 아니다) 카드만 봐서는 어느 쪽이든 왜 걸렸는지 알 수 없다.

오프라인 스냅샷에 본문이 없다는 사정은 원문을 빼는 근거로 쓰지 않는다. 스냅샷이 갖는 것은 목록 항목(`Posting`)뿐이라 키워드도 똑같이 없기 때문이다. 그것은 검색 대상 필드의 문제가 아니라 오프라인 검색을 어떻게 할지의 문제이고, 아래 「서버 필터가 대체하는 범위와 오프라인에 남는 범위」에서 다룬다.
...
`Posting.dueDate` 는 nullable 이다. 마감일을 못 뽑은 공고가 실제로 온다는 뜻이고, 프리셋마다 그 취급이 다르다. 「전체」와 「마감 지난 공고 포함」은 남기고, 「7일 이내」·「30일 이내」·「직접 지정」은 제외한다.
...
서버가 UTC 로 오늘을 잡으면 한국 시각 오전 9시 이전에는 서버의 오늘이 기기보다 하루 뒤처진다. UTC 날짜가 한국 날짜보다 앞설 일은 없으니 어긋남은 늘 한 방향이고, 그때 「7일 이내」는 어제부터 엿새 뒤까지를 훑는 조회가 된다.
...
검색은 다르다. 서버가 파싱 키워드까지 뒤지기로 하면 `Posting` 에는 `keywords` 가 없어서 오프라인이 그 규칙을 흉내 낼 수 없다. 같은 검색어가 온라인과 오프라인에서 다른 목록을 내는데, 사용자에게는 둘 다 「검색 결과」로 보인다. 그래서 오프라인에서는 검색을 잠그고 스냅샷 전체를 보여 주자는 것이 이 문서의 제안이다. 지금 오프라인이 막는 것은 북마크(`feed_offline_read_only`)와 이어 읽기(`FeedViewModel.loadMore` 가 `isOffline` 이면 그 자리에서 돌아선다) 둘이고 검색어 입력은 그대로 열려 있으므로, 잠금과 안내 문구는 #159 에서 함께 만든다.
