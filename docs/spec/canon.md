...
| 추천 이유(`reason`) 타입이 한 응답 안에서 갈림 — `topPick.reason` 은 배열, `byStrength[].reason` 은 문자열 | API_SPEC §7 안 | **스키마 그대로 받는다.** 서버가 실제로 보내는 것이 계약이고, 앱에서 한쪽으로 맞추면 반대쪽이 파싱 단계에서 실패한다. 하나로 모으는 일은 도메인 변환이 한다(`feature:foryou:data` 의 `ForYouMapper` — 문자열 하나는 원소 하나짜리 목록). 어느 쪽으로 정할지는 서버 몫이라 BE 저장소에 알렸다 | [#190](https://github.com/Team-CareerCompass/CareerCompass-FE/issues/190) · [BE #55](https://github.com/Team-CareerCompass/CareerCompass-BE/issues/55) |
