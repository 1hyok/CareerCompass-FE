...
| 온보딩 Step 1~4 | 하단 실패 배너 | 버튼 잠금(진행 표시 없음) | — | 없음 | 하단 실패 배너 | **배너만 — 나갈 길 없음 ⚠️ #211** |
...
| 게시판 등록 — 제출 | 점검 스낵바(`BoardRegisterMessage.Maintenance`) | `feed_maintenance_title` = 서비스가 잠시 점검 중이에요 | 스낵바라 버튼 없음 — 감지 결과와 폼은 그대로 남는다 |
...
정확히는 401 도 `toFeedFailureReason()` 의 `else` 로 떨어져 `Generic` 실패 상태가 **함께** 실린다. 다만 같은 자리에서 `sessionEnded` 가 켜지고(`FeedViewModel.recordFailure`) Entry 의 `LaunchedEffect` 가 곧바로 화면을 걷어내므로 그 실패 화면은 사용자에게 남지 않는다. **이동 배선이 곧 답이고, 사유 분기는 답이 아니다** — 온보딩은 사유를 `SessionExpired` 로 정확히 좁혀 놓고도 이동이 없어 막다른 길이 된다(#211).
...
| 온보딩 Step 1~4 | ⚠️ **배너만 뜨고 이동이 없다**(#211) — `OnboardingFailureReason.SessionExpired` 로 같은 문구를 하단 배너에 그릴 뿐, `onSessionEnded` 를 부르는 경로가 없다 | `onboarding_failure_session_expired` = 로그인이 만료됐어요. 다시 로그인해 주세요 |
| 온보딩 문서 업로드 카드 | 카드 상태 줄만 바뀐다 | `onboarding_upload_failed_session_expired` = 로그인 만료 |
...
| 온보딩 Step 1~4 | 아니다 — 배너가 화면 위에 얹힌다 | 입력값(사용자가 고칠 수 있다) | 입력 조건은 살아 있다 ✅ / **세션 만료만 막다른 길** ❌ #211 |
