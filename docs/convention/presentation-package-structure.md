...
|---|---:|---|---|---:|
| `careercompass` | 116 | `author` · `shared` · `reporting` | 기능별 | 6 |
| `foryou` | 79 | `component` · `screen` · `viewmodel` · `model` · `mapper` · `util` · `hometab` · `navigation` | 타입별 | 2 |
| `feed` | 65 | `home` · `detail` · `summary` · `recordsbox` · `deliveryverification` · `playlist` · `careercompasslist` · `senderdetail` · `navigation` · `reporting` | 기능별 | 2 |
| `profile` | 64 | `component` · `screen` · `viewmodel` · `social` · `navigation` | 타입별 | 1 |
| `editor` | 38 | `component` · `screen` · `viewmodel` · `navigation` | 타입별 | 2 |
| `onboarding` | 27 | `signup` · `login` · `findaccount` · `terms` · `navigation` · `reporting` | 기능별 | 2 |
| `home` | 5 | `usecase` · `reporting` | 평면 | 1 |

7개 모듈 중 3개(파일 수로는 208/394)가 이미 기능별이고, 그중엔 가장 큰 `careercompass` 가 있다. 소수를 다수에 맞춘다.
...
- **`internal` 이 실제로 쓰인다** — 기능별인 `careercompass` 는 top-level 선언 294개 중 56개가 `internal` 이다. 타입별인 `profile` 은 119개 중 4개뿐이다. 폴더가 경계를 안 그으면 가시성도 안 좁혀진다.
- **타입별은 스스로도 일관되지 않는다** — `foryou` 는 `DiaryWriteUiState.kt` 가 `viewmodel/` 에, 같은 성격의 `DailyDiary.kt` 는 `model/` 에 있다. 홈 탭은 `hometab/HomeTabMindRecordLazyItems.kt` 와 `component/hometab/RecordCategoryCard.kt` 로 갈렸다. 규칙이 없으니 어디를 봐야 할지 매번 달라진다.
...
`careercompass` 는 기능별이지만 `shared/body/infinite/content/list/item/` 까지 6단계로 내려간다. 반대 방향으로 불편해진 사례이고, 별도로 평탄화한다.
...
```bash
M=feature/profile/presentation/src/main/kotlin/com/cambridge/feature/profile/presentation
...
- **그 모듈에 열린 PR 이 없을 때 진행한다.** 이동은 리베이스가 사실상 불가능하다. 착수 전에 확인한다.
  ...
  gh pr list --state open --limit 100 --json number,headRefName,files \
    -q '.[] | select(any(.files[]; .path | startswith("feature/profile/presentation/"))) | "\(.number) \(.headRefName)"'
  ```
...
- [x] 규칙 문서화 — 이 문서
- [ ] [#1086](https://github.com/1hyok/CareerCompass-FE/issues/1086) — `feature/editor/presentation` (38, 가장 작음 → 파일럿) — @koongmai
- [ ] [#1087](https://github.com/1hyok/CareerCompass-FE/issues/1087) — `feature/foryou/presentation` (80) — @Sadturtleman
- [ ] [#1088](https://github.com/1hyok/CareerCompass-FE/issues/1088) — `feature/profile/presentation` (64) — @koongmai
- [ ] `feature/careercompass/presentation` 깊이 평탄화 검토 (117, 이미 기능별이므로 이관 아님) — @1hyok

`feed` · `onboarding` 은 이미 규칙을 만족해 대상이 아니다. `home` 은 파일 5개로 아직 폴더를 나눌 규모가 아니며, 화면이 늘면 R1 을 따른다.
