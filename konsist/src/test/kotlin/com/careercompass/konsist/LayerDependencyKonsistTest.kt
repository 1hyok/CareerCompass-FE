 ...
 *
 * domain 패키지(`com.careercompass..domain..`) 파일의 import 만 검사한다.
 * - 허용: `core:model`(`com.careercompass.core.model.*`), 같은 domain 레이어(`com.careercompass..domain..`; cross-domain 포함)
...
class LayerDependencyKonsistTest {
    private fun domainFiles() = Konsist.scopeFromProject().files.withPackage("com.careercompass..domain..")
    ...
    private companion object {
        ...
        const val CAREERCOMPASS_PREFIX = "com.careercompass."

        /** data / presentation 레이어 패키지. */
        val FORBIDDEN_LAYER_IMPORT = Regex("""^com\.careercompass\..*\.(data|presentation)\..*$""")

        /** core:model 또는 같은 domain 레이어만 허용 (그 외 com.careercompass.* 는 위반). */
