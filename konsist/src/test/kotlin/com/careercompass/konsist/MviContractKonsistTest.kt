 ...
 * MVI 계약 회귀 가드 (#244).
 *
 * 애프터노트의 MVI 전환(Afternote/Afternote-FE #1801)에서 그대로 들여왔다. 강제 없이 시작하면
 * 반쪽 도입으로 파편화만 늘어난다 — 이 저장소의 ViewModel 11개는 단일 `UiState` 노출은 정착했지만
 * 전부 `MutableStateFlow` 를 직접 들고 `onXxxConsumed()` public fun 으로 일회성 신호를 비운다.
 * 규약을 문서로만 두면 새 화면이 같은 모양으로 늘어난다.
 *
 * 규칙은 셋이다.
...
class MviContractKonsistTest {
    ...
    fun `MviViewModel 상속체는 상태 홀더를 직접 선언하지 않는다`() {
        val violations = stateHolderDeclarations(productionFiles)

        check(violations.isEmpty()) {
            buildString {
                appendLine("MviViewModel 상속체가 상태 홀더를 직접 선언한다 (${violations.size}건).")
                appendLine("전이가 reduce 밖으로 새면 베이스를 상속한 의미가 없다 (#244).")
                appendLine()
                violations.sorted().forEach { appendLine("  $it") }
                appendLine()
                appendLine("상태는 dispatch(ReducerEvent) 로만 바꾸고, 일회성 신호는 UiState 의")
                appendLine("nullable 필드로 흡수한다 . 기준: docs/convention/mvi.md")
            }
        }
    }
    ...
    fun `Intent 와 ReducerEvent 는 sealed interface 다`() {
        val violations = openContractTypes(productionFiles)
    ...
    /** 실제 Kotlin 파서로 작은 회귀 fixture 를 읽는다. */
    private fun fixtureFiles(root: File): List<KoFileDeclaration> {
        check(root.isDirectory) { "Konsist fixture 디렉터리가 없다: $root" }
        return Konsist.scopeFromExternalDirectories(listOf(root.path)).files
    }

    /** 프로덕션(비-test) 소스셋의 파일. 다른 가드와 같은 `scopeFromProduction()` 기준이다. */
    private val productionFiles: List<KoFileDeclaration> by lazy { Konsist.scopeFromProduction().files }
    ...
    private companion object {
        ...
        private val ISSUE_245_ONBOARDING =
            setOf(
                "com.careercompass.feature.onboarding.presentation.biometric.BiometricEnrollViewModel",
                "com.careercompass.feature.onboarding.presentation.biometric.BiometricLoginViewModel",
                "com.careercompass.feature.onboarding.presentation.flow.OnboardingViewModel",
                "com.careercompass.feature.onboarding.presentation.login.LoginViewModel",
            )
        ...
        /**
         * 전환 전 9개. `app` 의 ViewModel 2개(`MainViewModel`·`MyTabPlaceholderViewModel`)는 이 규칙의
